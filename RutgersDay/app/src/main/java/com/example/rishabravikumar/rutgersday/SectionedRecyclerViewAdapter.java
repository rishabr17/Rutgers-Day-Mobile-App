...
import com.example.rishabravikumar.rutgersday.Section;
import java.util.ArrayList;
import java.util.LinkedHashMap;
import java.util.Map;
import java.util.UUID;
import java.util.List;
import static com.example.rishabravikumar.rutgersday.Section.State;
import android.app.Activity;
import android.util.Log;

public class SectionedRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements RecyclerViewFastScroller.BubbleTextGetter {
    ...
    public void removeAt(int position, Activity activity) {
        Log.d("mDataArray", Integer.toString(position));
        /*for(int i = 0; i < mDataArray.size(); i++) {
            if(((MainActivity)activity).itineraryCards.get(i).name.equals(mDataArray.get(position).name)) {
                ((MainActivity) activity).itineraryCards.remove(i);
                break;
            }
        }*/
    ...
    public String getTextToShowInBubble(int position) {
        ...
        String name = mDataArray.get(position).getName();
        if (name == null || name.length() < 1)
            return null;

        return mDataArray.get(position).getName().substring(0, 1);
    }
